<#import "template/template.ftl" as frame> <#global
menu="${typeStr}套餐列表"> <@frame.html title="${typeStr}套餐列表" left=false
nav=false js=["homepage/js/index.js","homepage/js/unslider.min.js"]
			...
			<div class="fn-left pg-uplan-product-item mr15 mt15">
				<a href="${rc.contextPath}/product/d/${prod.id}">
					<dl class="bg-color-white">
						...
						<dd>
							<span class="mr20">曝光次数</span> <span><em>${prod.playNumber}</em>/天</span>
						</dd>
						<dd>
							<span class="mr20">展示期限</span> <span> <#if (prod.days % 30
								== 0)> <em>${prod.days/30}</em>月 <#elseif (prod.days % 7 == 0)>
								<em>${prod.days/7}</em>周 <#else> <em>${prod.days}</em>天 </#if>
							</span>
						</dd>
						<dd>
							<span class="mr20">金额</span> <span><em><#setting
									number_format=",###">${prod.price}</em>元</span>
						</dd>
					</dl>
				</a>
				<p class="J_click_p" data="218">
					<span id="J_count_time_b" data2="0" data1="328261"> <a
						href="${rc.contextPath}/order/iwant/${prod.id}">马上预定</a>
					</span> <a class="text" href="${rc.contextPath}/product/d/${prod.id}">（查看详情）</a>
				</p>
			</div>
			</#list>
		</div>
