		...
		generateTemplate = function( settings ) {
			...
			if ( !hideDays ) {
				if(daysLeft > 0){
					...
					//处理如果是0天不显示
					template = template.replace(settings.daySingularText,"");
					template = template.replace(settings.dayText,"");
				}
			}
			if ( !hideHours ) {
				//只有当小时是0且天是0时 去掉0小时显示
				if(_hiddleZero && hrsLeft ==0){
					template = template.replace(settings.hourSingularText,"");
					template = template.replace(settings.hourText,"");
				}else {
