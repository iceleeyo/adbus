...
public class OrderDataServiceImpl implements OrderServiceData {
	...
	public Page<JpaOrders> getAllOrders(String productId, int page, int pageSize) {
		if (page < 0)
            page = 0;
        if (pageSize < 1)
            pageSize = 1;
        ...
        if (productId == null || StringUtils.isEmpty(productId)) {
            ...
            Predicate query = QJpaOrders.jpaOrders.product.id.like("%" + productId + "%");
            return orderRepo.findAll(query, p);
        }
	}

	public Page<JpaOrders> getValidOrders(int page, int pageSize) {
		if (page < 0)
            page = 0;
        if (pageSize < 1)
            pageSize = 1;
